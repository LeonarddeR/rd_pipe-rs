...
pub fn dll_path() -> PathBuf {
	...
	if !target_triple.is_empty() {
		target_dir.join(target_triple).join(profile).join("rd_pipe.dll")
	} else {
		default_path
	}
}
...
impl HkcuOverride {
	pub fn new() -> io::Result<Self> {
		...
		let temp =
			tempfile::Builder::new().prefix("rd_pipe_test_hive_").suffix(".dat").tempfile()?;
...
impl DllHandle {
	...
	pub fn load() -> &'static DllHandle {
		...
		HANDLE.get_or_init(|| {
			...
			DllHandle { get_class_object, _lib: lib }
		})
	}
...
pub fn create_plugin(dll: &DllHandle) -> IWTSPlugin {
	...
	assert!(!factory_ptr.is_null(), "factory is null after DllGetClassObject");
