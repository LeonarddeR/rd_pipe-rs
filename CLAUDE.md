...
cargo bindgen                                      # regenerate src/bindings.rs + tests/common/bindings.rs
```
...
## Bindings (windows-rs 0.100)

Runtime dependencies on windows-rs are `windows-core` and `windows-registry` only; the `windows` umbrella crate is not used. Every Win32 function, constant, struct and COM interface the crate touches comes from `src/bindings.rs`, generated by `windows-bindgen` and committed. The integration tests have their own `tests/common/bindings.rs`. Never edit either file by hand.

- `tools/bindgen/` is a standalone crate (own `Cargo.lock`, not a workspace member). `cargo bindgen` (alias in `.cargo/config.toml`) runs it from the repo root; it reads the command files `tools/bindgen/lib.txt` and `tools/bindgen/tests.txt` (`--out`, `--implement`, `--filter`). To use a new API, add its name to the `--filter` list and rerun. Interfaces the crate implements must be listed under `--implement` with their fully qualified name (`Windows.Win32.IWTSPlugin`); a bare name silently emits no `_Impl` trait.
- Layout is the default namespaced one, so paths read `crate::bindings::Windows::Win32::CreateNamedPipeW`, mirroring `windows::Win32::CreateNamedPipeW` in the umbrella crate.
- The two `mod bindings;` declarations carry `#[expect(...)]` for exactly the lints the generated code triggers (Win32 naming, `clippy::missing_transmute_annotations`, `clippy::too_many_arguments`). When a regeneration changes that set, clippy reports an unfulfilled expectation or a new warning; adjust the list rather than widening it to `clippy::all`.
- Generated shapes follow the `windows` 0.100 conventions: functions return raw `BOOL`/`HANDLE`/`u32` (use `.ok()?`, check the documented sentinel, then `Error::from_thread()`), most constants are `i32` and get cast at the call site, Win32 error codes convert through `WIN32_ERROR(code as u32)`, kernel handles live in `std::os::windows::io::OwnedHandle`, and `LocalAlloc` memory in the private `LocalMem` guard of `security_descriptor.rs`.
- Until a published `windows-default` contains `IWTSVirtualChannel` (the `tsvirtualchannels.h` metadata landed on windows-rs master in #4882, after 0.100.0 shipped), `tools/bindgen/Cargo.toml` patches `windows-default` to a pinned windows-rs commit. Once it is published: remove that `[patch.crates-io]` table, rerun `cargo bindgen` (refreshes the tool lockfile), delete `tools/bindgen/deny.toml` (it only exists to allow the git source) and point the tool's cargo-deny step in `security-audit.yml` at the root `deny.toml`. The DLL's own lockfile never contains a git source.
- CI job `Check generated bindings` reruns the tool and fails on a diff; `security-audit.yml` runs cargo-deny for the tool with `tools/bindgen/deny.toml`, which allows the windows-rs git source.
...
- `bindings.rs` — generated `windows-bindgen` output (see Bindings above); regenerate with `cargo bindgen`.
- `class_factory.rs` — `IClassFactory` impl producing `IWTSPlugin`.
- `overlapped.rs` — event creation into `std::os::windows::io::OwnedHandle` (sentinels checked via `HandleOrNull`/`HandleOrInvalid`), `Shutdown` signal, `wait_overlapped` (op event + shutdown event pair, `CancelIoEx` on shutdown), `run_overlapped` (issue op, consume sync completion or await pending).
...
- `security_descriptor.rs` — logon SID lookup and SDDL → `SecurityAttributes` conversion (derefs to `SECURITY_ATTRIBUTES`; the descriptor is freed when the value drops).
