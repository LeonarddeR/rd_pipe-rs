...
- Pipe creation is a single synchronous attempt in `OnNewChannelConnection` that refuses the channel on failure. Master's accept-then-retry-every-100ms loop was rejected after a source-level deep dive of both ends: the loop's founding transient (per-reconnect instance re-creation, b05dbef 2022) was eliminated by create-once in b101562; every remaining failure is persistent and the loop re-submits the same doomed pipe name forever (the name embeds the channel object's address), whereas a refusal fails the server-side `WTSVirtualChannelOpenEx` observably and the next open gets a fresh channel object and thus a fresh name — the only path that escapes a name collision. RDAccess (verified in LeonarddeR/rdAccess `main`) re-attempts opens event-driven (focus/unlock/startup rescans) and abandons an accepted channel after its 3 s XON timeout anyway, so the background retry can never rescue a channel; it only leaks zombie DVC handles server-side and spinning 10 Hz threads client-side. If field logs ever show a demonstrated transient (non-`ACCESS_DENIED`, e.g. `ERROR_NO_SYSTEM_RESOURCES`, where an immediate reopen succeeds), the sanctioned fix is a bounded in-place retry (≤3 attempts, ≤500 ms) for that error class only — never an unbounded loop, and never accepting the channel before the pipe exists.
