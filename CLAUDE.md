...
- Generated shapes follow the `windows` 0.100 conventions: functions return raw `BOOL`/`HANDLE`/`u32` (use `.ok()?`, check the documented sentinel, then `Error::from_thread()`), constants are `i32` and get cast at the call site, Win32 error codes convert through `WIN32_ERROR(code as u32)`, and owned allocations live in a `Drop` type (`LocalMem` in `security_descriptor.rs`).
- Until a published `windows-default` contains `IWTSVirtualChannel` (the `tsvirtualchannels.h` metadata landed on windows-rs master in #4882, after 0.100.0 shipped), `tools/bindgen/Cargo.toml` patches `windows-default` to a pinned windows-rs commit. Remove that `[patch.crates-io]` table and rerun `cargo bindgen` once it is published. The DLL's own lockfile never contains a git source.
...
- `security_descriptor.rs` — logon SID lookup and SDDL → `SECURITY_ATTRIBUTES` conversion (caller must `LocalFree` the descriptor, e.g. via `LocalMem`).
