...
- Moved from the `windows` 0.62 umbrella crate to `windows-core` and
  `windows-registry` 0.100. The Win32 and COM declarations the crate uses
  are generated by `windows-bindgen` into `src/bindings.rs` (and
  `tests/common/bindings.rs` for the integration tests) by the new
  `tools/bindgen` helper; `cargo bindgen` regenerates them and CI fails
